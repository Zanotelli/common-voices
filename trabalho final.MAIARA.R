...
best_train <- NULL  # Adicione esta linha para armazenar o melhor conjunto de treino
best_test <- NULL   # Adicione esta linha para armazenar o melhor conjunto de teste
...
for (j in seq_len(10)) {
  ...
  for (h in h_values) {
    ...
    if (aux > best_accuracy) {
      ...
      best_accuracy <- aux
      if(j-1>0){
        if(best_accuracy > accuracy[j-1, ]){
          train_model <- best_train
          test_model <- best_test
          h_model <- best_h
        }
      }
    }
...
#-------------------------------------------------------
#treina os dados 
x_train <- x[train_model, ]
y_train <- y[train_model]
x_test <- x[test_model, ]
y_test <- y[test_model]
